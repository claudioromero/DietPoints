...
namespace CloudX.DietPoints.Web.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        public async Task<ActionResult> Login(LoginViewModel model, string returnUrl)
        {
            ...
            var enabled = Request.GetOwinContext().Get<ModelDbContext>().Users.Any(x => x.UserName == model.Email);
        ...
        public async Task<ActionResult> Register(RegisterViewModel model)
        {
            if (ModelState.IsValid)
            {
                var result = await UserManager.Register(model.Email, model.Password);
                if (result.Succeeded)
                {
                    await SignInManager.SignInAsync(result.User, false, false);
